...
/**
 * The controller class for managing album actions.
 *
 * @author Jimmy Han, Luthfi Jamal Mohamed
 */
public class AlbumController extends LogoutController {
    ...
    public void addPhoto(ActionEvent event) throws IOException {
        ...
        if (!Photos.session.getCurrAlbum().addPhoto(newPhoto)) {
            showAlert("ERROR", "Photo already exists.", AlertType.ERROR);
            return;
        } else {
    ...
    public void addCaption(ActionEvent event) throws IOException {
        ...
        if (newCaption.isEmpty() || newCaption == null) {
            showAlert("ERROR", "Please enter a caption.", AlertType.ERROR);
            return;
        }
    ...
    public void addTag(ActionEvent event) throws IOException {
        ...
        if (name.isEmpty() || value.isEmpty()) {
            showAlert("ERROR", "Please fill in both the name and value fields.", AlertType.ERROR);
            return;
        }
        Tag tag = new Tag(name, value);
        Photos.session.getCurrAlbum().setPhoto(index);
        if (Photos.session.getCurrAlbum().getPhoto().hasTag(tag)) {
            showAlert("ERROR", "Tag already exists.", AlertType.ERROR);
            return;
        } else {
    ...
    public void deleteTag(ActionEvent event) throws IOException {
        ...
        if (Photos.session.getCurrAlbum().getPhoto().hasTag(tag)) {
            ...
            showAlert("ERROR", "Tag does not exist.", AlertType.ERROR);
            return;
        }
        Session.serialize(Photos.session);
        refresh();
    ...
    public void movePhoto(ActionEvent event) throws IOException {
        ...
        if (Photos.session.getCurrUser().getAlbumNameList().contains(album)) {
            ...
            alert.setContentText("Are you sure you want to move this photo to " + album + "?");
            Optional<ButtonType> result = alert.showAndWait();
            if (result.get() == ButtonType.OK) {
                int index = photos.getSelectionModel().getSelectedIndex();
                Photos.session.getCurrAlbum().setPhoto(index);
                ...
                Photos.session.getCurrUser().findAlbum(album).addPhoto(photo);
                Photos.session.getCurrAlbum().deletePhoto(index);
                Album.serialize(Photos.session.getCurrUser().findAlbum(album));
                Album.serialize(Photos.session.getCurrAlbum());
                refresh();
            }
        } else {
    ...
    public void changeCaption(ActionEvent event) {
        int index = photos.getSelectionModel().getSelectedIndex();
        if (index < 0) {
            showAlert("ERROR", "No photo is selected", AlertType.ERROR);
            return;
        }
        String newCaption = caption.getText().trim();
        caption.clear();
